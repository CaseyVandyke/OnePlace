import { getDogGuide } from "../data/dogGuides.js";
import "./DogGuide.css";
...
export default function DogGuide({ guideId, size = 54 }) {
  const guide = getDogGuide(guideId);
  const shape = breedShapes[guide.id];

  return (
    <svg
      className={`puppy-guide dog-guide dog-guide-${guide.id}`}
      width={size}
      height={size * 1.16}
      viewBox="0 0 100 116"
      aria-hidden="true"
    >
      <path className="puppy-tail" d={shape.tail} />
      ...
      <path className="puppy-body" d={shape.body} />
      ...
      <path className="puppy-ear puppy-ear-left" d={shape.leftEar} />
      <path className="puppy-ear puppy-ear-right" d={shape.rightEar} />
      <ellipse className="puppy-head" cx="50" cy="38" rx={shape.headRadiusX} ry={shape.headRadiusY} />
      {guide.id !== "labrador" && <path className="puppy-blaze" d="M50 11c-8 8-9 18-4 26l8 1c5-9 4-19-4-27Z" />}
